...
export class CreateTrackButton extends React.Component {
  ...
  onClick = () => {
    const { name, description, bandId } = this.props;

    if (this.props.canActiveArtistCreateTrackForBand) {
      this.props.createTrack({
        bandId,
        name: `${Math.random().toString(36).substring(7)} ${Math.random()
          .toString(36)
          .substring(4)}`,
  ...
  render() {
    return (
      <Button onClick={this.onClick} disabled={this.isDisabled()}>
        Create Track
      </Button>
    );
  }
...
export function mapStateToProps(state, { bandId }) {
  const canActiveArtistCreateTrackForBand =
    Selectors.usm.canActiveArtistCreateTrackForBand(state, bandId);
