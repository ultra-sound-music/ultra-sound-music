...
export class TokenCard extends React.Component {
  ...
  render() {
    const { entityId, tokenType, name, description, ctaButton } = this.props;
    ...
    return (
      <Card className={className}>
        <Card.Header className='TokenCard__header'>
          <Nav className='TokenCard__title'>{tokenType}</Nav>
          <Nav className='TokenCard__actionIcons'>
            {this.props.actionIcons &&
              this.props.actionIcons.map((icon, i) => (
                <div key={i} className='TokenCard__actionIcon'>
                  {icon}
                </div>
              ))}
          </Nav>
