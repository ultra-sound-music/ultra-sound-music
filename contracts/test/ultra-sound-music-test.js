...
describe('test USM tokens', async () => {
  const [deployer, wallet1, wallet2, wallet3, wallet4] = await provider.getWallets();
  ...
  before("deploy contracts", async () => {
    ...
    USMA = await factory_USMArtistToken.connect(deployer).deploy("USM Artist Token", "USM-A")
    await USMA.deployTransaction.wait()
    USMB = await factory_USMBandToken.connect(deployer).deploy("USM Band Token", "USM-B", USMA.address)
    await USMB.deployTransaction.wait()
    USMT = await factory_USMTrackToken.connect(deployer).deploy("USM Track Token", "USM-T", USMA.address, USMB.address)
    await USMT.deployTransaction.wait()

  });

  it("should mint an artist tokens for wallet 1-4", async() => {
    const USMA_w1 = factory_USMArtistToken.connect(wallet1).attach(USMA.address)
    const USMA_w2 = factory_USMArtistToken.connect(wallet2).attach(USMA.address)
    const USMA_w3 = factory_USMArtistToken.connect(wallet3).attach(USMA.address)
    const USMA_w4 = factory_USMArtistToken.connect(wallet4).attach(USMA.address)


    await USMA_w1.createArtist(testURI, {value: ethers.utils.parseEther(".15")})
    await USMA_w2.createArtist(testURI, {value: ethers.utils.parseEther(".15")})
    await USMA_w3.createArtist(testURI, {value: ethers.utils.parseEther(".15")})
    await USMA_w4.createArtist(testURI, {value: ethers.utils.parseEther(".15")})
